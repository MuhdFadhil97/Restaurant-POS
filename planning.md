...
- ⬜ 8.2 Receipts + cash drawer (checkout/finalize hooks, reprint, "No sale" drawer open + audit, POS printer-status dot)
- ⬜ 8.3 Kitchen tickets (station printer, Send to kitchen, auto-fire on checkout/QR)
- ⬜ 8.4 Customer display (BroadcastChannel + SSE relay, `/display/:token`)
- ⬜ 8.5 Print bridge agent (`print-bridge/`)
