...
- 🟡 **8.6 Terminal-local USB/Bluetooth — browser side done, Android plugin not started.** `GET /print-jobs/terminal/:terminalId` (claim) / `POST /print-jobs/:id/ack` let the terminal's own signed-in browser session deliver jobs for printers plugged into it (JWT-authenticated, not a bridge token; rejects a job that isn't `TERMINAL_LOCAL`). `frontend/src/lib/hardware/localPrinter.ts` has WebSerial and WebUSB drivers behind feature detection — both are secure-context-only APIs (no `http://<lan-ip>`, only HTTPS or `http://localhost`), so an unsupported origin shows a message instead of crashing (same lesson as the `crypto.randomUUID()` bug in 8.2). Printer form's "USB / Bluetooth on a terminal" option is enabled. Verified: claim/ack/retry over curl, and the graceful-degradation vs. connect-buttons split across an insecure LAN origin and `http://localhost` in the browser. **Not verified: an actual paired printer** — needs real hardware, which this environment doesn't have. **Not started: the Capacitor plugin in `../Restaurant-POS-android`** for the packaged tablet app, which needs native Android USB/Bluetooth code this environment can't build or test.
...
| 2026-09-24 | 8.5 committed (596a642). 8.6 browser-side (WebSerial/WebUSB claim+ack) built and verified without real hardware; Android Capacitor plugin still to do. |
